...
ITMOScript - динамически типизированный скриптовой язык, созданный как учебный проект. В языке есть поддержка чисел и арифметики, строк, списков и функций.
...
Функциям нельзя присвоить другое значение после создания, то есть такой код вызовет RuntimeError:
```
...
* `type_of(x)` - возвращает строку с названием типа переменной `x`.
